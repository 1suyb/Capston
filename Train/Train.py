...
class Train:
    def __init__(self, load_pretrained=False,
        ...
        def Train_D():
            ...
            model_dis = Discriminator(input_shape=(channels, *self.hr_shape))
            ...
            for epoch in range(self.n_epoch):
                ...
                for batch_idx, imgs in enumerate(tqdm_bar):
                    ...
                    train_counter.append(
                        batch_idx * batch_size + imgs_lr.size(0) + epoch * len(self.train_dataloader.dataset))
                ...
                if np.argmin(test_gen_losses) == len(test_gen_losses) - 1:
                    torch.save(model_gen.state_dict(), f"./saved_models/{self.model}/generator_cr2.pth")
                    torch.save(model_dis.state_dict(), f"./saved_models/{self.model}/discriminator_cr2.pth")
            ...
            for epoch in range(self.n_epoch):
                ...
                for batch_idx, imgs in enumerate(tqdm_bar):
                    ...
                    if batch_idx == 1:
                        ...
                        imgs_lr = make_grid(imgs_lr, nrow=1, normalize=True)
                        imgs_grid = torch.cat((imgs_hr, imgs_lr, imgs_sr), -1)
                        save_image(imgs_grid, f"./images/{self.model}/{epoch + 21}.png", normalize=False)
                    test_gen_losses.append(gen_loss / len(self.test_dataloader))
                    test_disc_losses.append(disc_loss / len(self.test_dataloader))
                    if np.argmin(test_gen_losses) == len(test_gen_losses) - 1:
                        torch.save(model_gen.state_dict(), f"./saved_models/{self.model}/generator_cr.pth")
